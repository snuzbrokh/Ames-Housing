...
def add_features(df):
	...
	df['Age'] = df.YrSold.astype(np.int64) - df.YearRemodAdd
	df['logPrice'] = np.log(train['SalePrice'])
	hood_means = df.groupby(by=["Neighborhood",'TotalBathrooms','KitchenAbvGr','ExterQual','OverallQual','MSZoning'])["logPrice"].mean().reset_index()
	hood_means['PriceRange'] = pd.qcut(hood_means.logPrice,5,labels=['Poor','LowerMiddle','Middle','UpperMiddle','Rich'])

	#df = df.merge(hood_means[['Neighborhood','TotalBathrooms','KitchenAbvGr','PriceRange','OverallQual','ExterQual','MSZoning']], on = ['Neighborhood','KitchenAbvGr','TotalBathrooms','OverallQual','ExterQual','MSZoning'])
	#df['PriceRange'] = a['PriceRange']

	#df.sort_values('PriceRange', inplace=True)
