...
```{r}
ames_num = ames_train %>% select_if(is.numeric)
```


```{r}
x_train = model.matrix(SalePrice ~ GrLivArea #+ sqrt(TotalBsmtSF) 
              + OverallQual + YearBuilt 
              + Fireplaces + TotalPorchSF 
              + MSSubClass + Neighborhood, ames_train)
y_train = ames_train$SalePrice


x_test = model.matrix(SalePrice ~ GrLivArea #+ sqrt(TotalBsmtSF) 
              + OverallQual + YearBuilt 
              + Fireplaces + TotalPorchSF 
              + MSSubClass + Neighborhood, ames_train)
...
### General GAM Model looking at simple interactions
```{r}
ames.gam <- mgcv::gam(log(SalePrice) 
                ~ s(GrLivArea) # Above Ground Square Footage
                + s(OverallQual) + s(YearBuilt) 
                + s(GarageArea) # Garage Predictors
                + s(TotalBsmtSF, by=BsmtQual)
                + s(MSSubClass, by = AfterWW2) 
             ...
             #   + s(LotArea) + s(LotFrontage) +s(OpenPorchSF) # + AfterWW2 + Neighborhood, 
                ,method='ML',data=ames_train)

ames.lm <- lm(log(SalePrice) 
                ~ sqrt(GrLivArea) #+ sqrt(TotalBsmtSF) 
              + OverallQual + YearBuilt 
              + Fireplaces + TotalPorchSF 
              + MSSubClass + Neighborhood:OverallCond
...
hist(gam.errors, breaks=20)
hist(lm.errors, breaks=20)
...
print(ames.gam$formula)
print(ames.lm$deviance)
```

```{r}
ames.gam <- mgcv::gam(log(SalePrice) 
                ~ ti(FirstFlrSF, by=KitchenAbvGr) + ti(SecFlrSF) 
                + ti(FirstFlrSF,SecFlrSF)
                + s(TotalBsmtSF)
                
                + s(OverallQual) + s(YearBuilt) 
                + s(GarageArea) # Garage Predictors
                #+ s(TotalBsmtSF, by=BsmtQual)
                + ti(MSSubClass, SecFlrSF)
...
submission = mutate(submission, Id = Id + 1460)
head(submission)
#write.csv(gam.test_predictions,'./data/price_predictions.csv')
```

```{r}
write.table(submission,file="./data/predictions.csv",col.names = c("Id","SalePrice"),sep = ",",row.names = F)
```
...
ames_train %>% 
  ...
  tally() %>% 
  filter(n > min_n) %>% 
  ggplot(aes(x=Neighborhood, y=OverallQual)) + 
  geom_point() +
  geom_text(color='red',size=4,aes(y=OverallQual+0.2, label=n)) +
  theme_bw() +
  theme(axis.text.x=element_text(angle=45, hjust=1)) 
