...
ames.test = ames %>% mutate(OrdMaster = log(QualCond*ExterCond*ExterQual))
ggplot(ames.test,aes(x=OrdMaster, y=logPrice)) + geom_point()
```
...
gam.anal %>% 
  ...
  labs(x = 'Train Set Index', y = 'GAM Standardized Residuals') +
  theme_bw()
...
lm.anal %>% 
  ...
  labs(x = 'Train Set Index', y = 'LM Standardized Residuals') +
  theme_bw()
```
