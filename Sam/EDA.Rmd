...
```{r}
ames_train$BsmtQual = as.factor(ames_train$BsmtQual)
library(caret)
preProcValues <- preProcess(ames_train, method = c("center"))

ames_train <- predict(preProcValues, ames_train_)
ames_test <- predict(preProcValues, ames_test_)
```
...
```{r}
ames.gam <- gam(log(SalePrice) 
                ~ s(GrLivArea) #+ s(TotalBsmtSF) 
                + s(OverallQual) + s(YearBuilt) 
                + Fireplaces + s(TotalPorchSF) 
                + s(MSSubClass) + Neighborhood:OverallQual #+ BsmtQual:TotalBsmtSF + ExterCond:(sqrt(GarageArea)+sqrt(LotArea))
...
print(paste('GAM: Mean Absolute Error:',mean(abs(gam.errors))))
print(paste('LM: Mean Absolute Error:',mean(abs(lm.errors))))
...
visreg(ames.gam)
visreg(ames.gam$fitted.values, ames_train$Neighborhood)
```
...
ames_train %>% 
  ...
  geom_point() +
  geom_text(color='red',size=4,aes(y=GarageCond+0.5, label=n)) +
  theme_bw() +
  theme(axis.text.x=element_text(angle=45, hjust=1)) 
