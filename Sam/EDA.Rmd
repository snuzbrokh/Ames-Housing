...
ames.gam <- mgcv::gam(log(SalePrice)
             ...
             #   + s(LotArea) + s(LotFrontage) +s(OpenPorchSF) # + AfterWW2 + Neighborhood,
                ,method='GCV.Cp', data=ames)

## Base GAM Model
ames.gam.base <- mgcv::gam(log(SalePrice)
                ~ s(GrLivArea, by=PriceRange, bs='cr', id=1) # Above Ground SF
                + s(TotalBsmtSF,bs='cr') # Bsmt SF
                + s(OverallQual, bs='cr', by=PriceRange)
                + s(YearBuilt, bs='cr')
...
print(paste('GAM BASE RMSE:',sqrt(mean(gam.base.errors^2))))

print(paste('GAM: Error in $ as Predicted:',round(mean(abs(gam.diff)),2)))
print(paste('GAM BASE: Error in $ as Predicted:',round(mean(abs(gam.base.diff)),2)))
